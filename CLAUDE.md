...
**Aucune écriture en base, sauf par recette versionnée sur trois tables.** Le compte MySQL est en lecture seule et le reste : c'est une décision, pas une limitation subie. Toute modification de tarif, de saison ou de restriction se fait par Thomas dans les écrans natifs de Vik, parce que leur enregistrement pousse vers Airbnb et Booking.com. Code lit, vérifie et constate ; il n'écrit ni par SQL à la main, ni par un plugin qui écrirait à sa place. Voir `convention-tarifs-annuelle.md`.

Exception décidée par Thomas le 26 septembre 2026 (`brief-f3-rappel-avant-sejour.md`, version 1.1), **pour ce périmètre seulement** : `sir_vikbooking_condtexts` (textes conditionnels), `sir_vikbooking_cronjobs` (gabarit des tâches planifiées) et `sir_vikbooking_adultsdiff` (suppléments par adulte). On y écrit uniquement par une recette versionnée dans le dépôt (`recettes-vik/`), exécutée par `appliquer-recette-vik.sh` : simulation par défaut, refus si l'environnement ne correspond pas, sauvegarde des lignes touchées et `--rollback`, idempotente, arrêt au premier écart. Motif : l'écran natif d'un texte conditionnel n'écrit que la ligne, sans autre effet (`admin/controller.php:15116-15222`, Vik 1.8.15). Code lance une recette sur la préproduction ; **Thomas seul la lance en production**, et chaque exécution en production se consigne dans `journal-vik.md`. Hors de ces trois tables, rien ne change.
