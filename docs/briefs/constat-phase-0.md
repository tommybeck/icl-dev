...
**Correctif du 22 septembre 2026 (`constat-fatal-page-paiement.md`) : le paragraphe suivant, tel qu'écrit ici le 11 septembre, était faux, et c'est cette erreur qui a fait planter tout paiement Stripe en phase 4.** Il affirmait : « Piège d'implémentation : l'appel est `do_action($hook, array(&$this))`, donc la fonction rappelée reçoit un tableau dont l'indice `0` est l'objet de paiement, et non l'objet directement. » C'est la lecture littérale du seul code de Vik (`payment.php:329`), correcte pour cette ligne prise isolément — mais elle ignore une couche supplémentaire, dans **WordPress lui-même** : `do_action()` (`wp-includes/plugin.php`) déballe automatiquement un argument de la forme `array($objet)` — un tableau à un seul élément qui est un objet — avant d'appeler les rappels, pour compatibilité ascendante avec le style PHP4 de `array(&$this)`. Relu à l'identique sur les deux environnements (préproduction et production), ce bloc n'est pas propre à Vik : c'est un mécanisme du cœur de WordPress, qui s'applique à n'importe quel `do_action($hook, array(&$this))`, de n'importe quel plugin.

**Ce qui est vrai : le rappel accroché sur `payment_before_begin_transaction_vikbooking` reçoit l'objet de paiement directement, jamais un tableau dont l'indice 0 le contiendrait.** Une fonction qui suppose le tableau et tente `isset($args[0])` sur l'objet réellement reçu lève une `Error` fatale sous PHP 8 (« Cannot use object of type X as array »), l'objet de paiement n'implémentant pas `ArrayAccess`. C'est exactement ce qui s'est produit : `mu-plugins/lme-brands/includes/payment-brand.php`, écrit en phase 4 sur la foi du paragraphe ci-dessus, a cassé le paiement des deux marques, sur les deux environnements, dès son activation — voir `constat-fatal-page-paiement.md` pour la trace complète et `docs/briefs/plan-de-marche.md` §B9c pour le correctif.

**La leçon : lire le seul code de Vik ne suffit pas à prédire le comportement réel d'un hook WordPress.** Une lecture ciblée d'un seul fichier, même exacte à la lettre, peut rester fausse une fois le cadre qui l'exécute ajouté. Ne pas répéter cette forme ailleurs dans ce chantier.
